...
function PropertyCard({ property, onEdit, onDelete, onClick }) {
  ...
  const summary = property.compliance_summary || {
    total: 0,
    compliant: 0,
    expiring_soon: 0,
    overdue: 0,
      ...
      <div className="flex items-start gap-4 mb-4">
        ...
        <div className="flex-1 min-w-0 pr-8">
          ...
          <p className="text-xs text-slate-400 mt-1">
            {property.postcode}
            {property.uk_nation ? ` • ${property.uk_nation}` : ''}
          </p>
        </div>
      </div>
        ...
        <div className={`flex items-center gap-1.5 px-2 py-1 rounded-full ${statusColors[status]?.bg}`}>
          ...
          <span className={`text-xs font-medium ${statusColors[status]?.text}`}>
            {status === 'compliant'
              ...
              ? `${summary.expiring_soon} due soon`
...
export default function PropertiesPage() {
  ...
  const handleDeleteProperty = async (propertyId) => {
    if (
      !window.confirm(
        'Are you sure you want to delete this property? This will also delete its linked compliance records and stored information.'
      )
    ) {
      return;
    }
  ...
  const handleSaveProperty = async (propertyData) => {
    try {
      if (editingProperty) {
        ...
        setProperties(
          properties.map((p) => (p.id === editingProperty.id ? response.data : p))
        );
      } else {
    ...
    <div data-testid="properties-page">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500">
            Add and manage the properties you want to keep compliant.
          </p>
        </div>
      ...
      {properties.length === 0 ? (
        <EmptyState
          ...
          description="Start with one property so you can track compliance records, store documents, and keep deadlines in one place."
          actionLabel="Add Property"
          onAction={handleCreateProperty}
          tip="A simple first step is enough. Add one property, then build out its compliance records."
          variant="featured"
        />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
